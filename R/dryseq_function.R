...
# To assess rhythmicity and mean differences of gene expression in RNA-Seq count data, we applied a statistical framework based on generalized linear models (Fig. S1A). As proposed (Love et al. 2014), we modeled counts Y that have been uniquely mapped to a gene g in a sample s as a negative binomial with a fitted mean μgs and a gene-specific dispersion parameter θg.


# To assess rhythmicity and mean differences of gene expression in RNA-Seq count data, we applied a statistical framework based on generalized linear models (Fig. S1A). As proposed (Love et al. 2014), we modeled counts Y that have been uniquely mapped to a gene g in a sample s as a negative binomial with a fitted mean μgs and a gene-specific dispersion parameter θg.

#The fitted mean is proportional to the quantity q of fragments that correspond to a gene in a sample scaled by a sample-specific scaling factor s (Love et al. 2014). This scaling factor depends on the sampling depth of each library and was estimated using the median-of-ratios method of DESeq2 (Anders and Huber 2010).

#A gene-specific distribution θg was estimated using empirical Bayes shrinkage (Love et al. 2014). Variance was computed from the following relationship to the dispersion parameter θ:

#The fit was performed using a generalized linear model with a logarithmic link function. Sample specific size factor (ss) was defined as an offset. The full GLM was defined as follows: μ is the raw count for gene g, condition c and Zeitgeber/circadian time t. α and β are coefficients of the cosine and sine functions, respectively. m is a coefficient to describe a mean expression level. When necessary, we included a batch specific m to account for technical batch effects. To select an optimal gene-specific model, we first assessed rhythmicity across the different conditions. To this end, we defined different models for four (e.g., Bmal1 WT, Bmal1 KO, Cry1/2 WT, Cry1/2 KO) or two conditions (i.e., PARbZip deficient (Hlf/Dbp/Tef KO) and proficient (Hlf/Dbp/Tef WT). Models were defined to have either zero (non-rhythmic pattern) or non-zero (rhythmic pattern) α and β coefficients for each analyzed condition. Moreover, for some models the values of α and β can be also shared within any combination of the four conditions (Fig. S1E-F). The coefficients α and β were used to calculate the phase ( ) and amplitude (log2-fold change peak-to-trough;  ) of a gene. Bayesian information criterion (BIC) based model selection was employed to account for model complexity (Kass and Raftery 1995) using the following formula:

                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               is defined as the log-likelihood of the model j from the regression, n is the number of data points and k is the number of parameters. To assess the confidence of the selected model j we calculated the Schwarz weight (BICW):
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               , with
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             (m* is the minimum BIC value in the entire model set).
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             We considered the BICWj as the confidence level for model j. The model with the highest BICW was selected as the optimal model within the set of all defined models. In a second iteration step (Fig. S1A), we set the coefficient α and β to the values of the selected model in the first regression. We then defined different models for the mean coefficient with differing or shared means between conditions (Fig. S1A, D). Each model was solved using generalized linear regression and each gene was assigned to a preferred model based on the BICW as described above for the first iteration.
#The fitted mean is proportional to the quantity q of fragments that correspond to a gene in a sample scaled by a sample-specific scaling factor s (Love et al. 2014). This scaling factor depends on the sampling depth of each library and was estimated using the median-of-ratios method of DESeq2 (Anders and Huber 2010).
